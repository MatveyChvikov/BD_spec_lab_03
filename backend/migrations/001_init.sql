...
COMMENT ON COLUMN users.email IS
    'TEXT: email переменной длины; целостность — UNIQUE + CHECK (regex).';
COMMENT ON COLUMN order_statuses.status IS
    'TEXT: код статуса; PK и цель FK из orders / order_status_history.';
COMMENT ON COLUMN orders.status IS
    'TEXT: текущий статус; FK на order_statuses(status).';
COMMENT ON COLUMN order_items.product_name IS
    'TEXT: наименование товара; CHECK на непустое значение.';
COMMENT ON COLUMN order_status_history.status IS
    'TEXT: статус в момент записи; FK на order_statuses(status).';
